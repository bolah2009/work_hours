module WorkHoursAuthenticatedAsHelper
  ...
  def authenticated_as_get_user(input, return_type:)
    case input
    when Proc
      parse_meta instance_exec(&input), return_type:
    when Symbol
      parse_meta instance_eval { send(input) }, return_type:
    else
      parse_meta input, return_type:
    end
  end
...
RSpec.configure do |config|
  %i[request system].each do |type|
    config.include WorkHoursAuthenticatedAsHelper, type:
  end
end
